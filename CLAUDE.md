...
- **Create** checks the branch (`branchExists`). If it is missing, `missingBranchPreflight` distinguishes an empty repository (no ref to branch from) from a merely missing `create_branch_from`. With `adopt_existing` (default true) it probes each path with `GetFileMetaData` - against the branch, or against the `create_branch_from` ref when the branch does not exist yet - and `adoptAwareActions` rewrites a `create` for an existing path into an `update`, plus a companion `chmod` when the exec bit differs (the commits API honours `execute_filemode` only on create and chmod; GitLab's docs claim chmod only, create is verified by `TestAccFiles_chmodCycle`). The adopt probe also fetches the content (one `GetFile` per existing path), and a path whose bytes and mode already match the plan yields no action; when nothing is left to commit, Create makes no commit and writes `commit_sha` null. A missing branch is materialised by `start_branch` on the commit itself (`start_sha` when `create_branch_from` is a 40/64-hex commit SHA; tags are not supported): GitLab creates the branch and the commit in one operation, so a server-side rejection (push rule, pre-receive hook) leaves no orphaned empty branch and CI sees one push event; the diagnostic for that failure names the branch and the `create_branch_from` ref. The one exception is the zero-action case, where there is no commit to attach `start_branch` to and the branch is created with a bare `CreateBranch` (`createBranch`). The branch check, preflight and probes run before the branch lock (they only read); the lock is taken for the re-check of the branch and the commit.
- **Read** is a no-op when `detect_drift = false`. Otherwise it fans out `GetFileMetaData` (HEAD-style, no body) at `refreshParallelism` (16) through an `errgroup`, compares `blob_id` and the exec bit with state, and pulls full content with `GetFile` only for files that actually drifted. A metadata response without a blob id is an error (it would compare as "unchanged" forever). A 404 drops the file from state; if *every* file 404s, or there are no files yet (right after import), it checks the branch and, when that is gone, removes the whole resource so the next apply can recreate it. Goroutines write only to their own slot of a pre-sized `[]fileRefreshResult`; a serial second pass mutates `state.Files` in path order. Whichever of `content` / `content_base64` the user chose is preserved on drift; binary drift into `content` is an error because cty would silently corrupt the bytes.
- **Update** calls `diffActions`: path gone from the plan -> `delete` (emitted first, so a path can turn from a file into a directory within one commit), new path -> create or adopt-update, content changed (`contentChanged`: cheap string compare first, bytewise fallback only for differing base64 or a form switch) -> `update`, exec bit flipped -> `chmod`. The adopt probe also fetches the remote content: a path whose bytes already equal the plan yields no action (only a `chmod` if the exec bit differs), so the import round-trip and a re-pointed resource make no commit. Zero actions means no commit; computed fields are carried over from state, or from the probe for adopted-identical paths (`carryOver`). A locked adoption whose probe carries no `last_commit_id` is an error, never a silently unlocked write. With `detect_drift = false` a file deleted out of band is still in state and the delete action fails with GitLab's 400 until `detect_drift` is re-enabled and a refresh runs (a refresh while it is false is a no-op); that asymmetry with Delete (which probes) is documented rather than papered over.
...
- **Import** takes `<project_id>::<branch>` and checks that the branch exists (a typo would otherwise only surface as an empty, unfixable resource); the files map starts empty and the next apply converges through adoption, without a commit when the repository already matches.
...
After every successful commit `stampBlobs` refreshes `blob_id` and `last_commit_id` for the paths the commit touched (`touchedPaths`) through a parallel metadata fan-out probing at `Ref = commitSHA` - the commit just created, **not** branch HEAD - so a racing writer's values can never be stamped next to our content. Untouched paths are not probed; `carryOver` keeps their state values, so a probe hiccup can never stamp the new commit's SHA next to a file it never modified. A probe failure, a header-less 2xx, or a `blob_id` longer than `maxBlobIDLen` (256, treated as hostile), leaves `blob_id` null with a warning; the next Read repopulates it. A nil commit or an empty commit id from a 2xx is an error without state on Create on purpose: returning state would make Terraform taint and replace the resource (a delete commit), while a re-run converges through adoption.

### Validators and schema conventions

Custom validators live in `schema_validators.go`; defaults use the framework's `booldefault.StaticBool` inline in the schema. `files` carries `mapNonEmpty()` on purpose: `files = {}` would mean "delete everything" on update, which is what `terraform destroy` is for. `mapKeysValidRepoPath` uses explicit segment logic instead of a character regex so legal git names like `..config` stay usable; only `.` / `..` segments, empty segments, NUL bytes and leading whitespace are rejected, the rest is GitLab's to validate. The same rules back `stringValidRepoPath` (the file data source's `file_path`); `stringBranchName` is the shared allowlist for `branch` and `create_branch_from` on the resource and `branch` on both data sources (it admits hex commit SHAs); `stringIsBase64` rejects a non-decodable `content_base64` at plan time. `content` / `content_base64` are exactly-one-of via `stringConflictsWithSibling` plus `objectFileContentRequired`, and neither is `Sensitive` by design (see SECURITY.md).
...
- `docs/` is generated by `tfplugindocs` (`//go:generate` in `main.go`) from schema descriptions plus `examples/provider/provider.tf`, `examples/resources/gitlabcommits_files/{resource.tf,import.sh}` and `examples/data-sources/<name>/data-source.tf`. The only template is `templates/index.md.tmpl`, which adds the Authentication and Limits sections to the provider page (the `token` description points at them); resource and data-source pages use tfplugindocs' defaults. Any schema, description or template change must be followed by `just docs` with the result committed; CI fails on drift.
- Go version bump checklist: `go.mod` (`go get go@1.NN.0`), `go get -u ./...` plus `go get -u tool` (the `tool` block and test-only imports are not covered by `./...`), `.golangci.yml` `run.go`, the golangci-lint `with: version:` pin in `ci.yml` (a prebuilt binary older than the Go release cannot type-check the module; nothing bumps `with:` inputs automatically), and the version text in README.md and CLAUDE.md.
...
- A `v*` tag triggers `.github/workflows/release.yml`, whose first job calls `ci.yml` through `workflow_call` (the gate is defined once; the tag run passes `CODECOV_TOKEN` explicitly and no other secret), then goreleaser (GPG-signed, `main.version` set via ldflags). The changelog is built from conventional commit titles (`feat:`, `fix:`, `docs:`; `chore:` and `test:` are excluded); there is no CHANGELOG file. Dependabot PRs use `fix(deps)` / `fix(ci)` with no `include: scope`: a second `(deps)` scope would miss the changelog's dependency group.
